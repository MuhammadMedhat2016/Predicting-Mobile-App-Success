...
def trim_data_set(data):
    ...
    trim(data, [trim_app_name,trim_install, trim_size, trim_price, trim_date], cols=['App Name', 'Installs', 'Size', 'Price', 'Last Updated'])
...
def run():
    ...
    # read data
    data = pd.read_csv('F:\\MlProject\\DataSets\\Project.csv')
    data = data.iloc[:, 0:11]
    # drop noisy data
    data.drop([6941, 12624, 18477], inplace=True)
    # Rearrange Data frame columns putting dependent rating column in the last..
    ...
    impute_data(data, ['most_frequent', 'mean', 'mean', 'mean', 'mean', 'most_frequent', 'most_frequent',
                      ...
                      [1, 2, 3, 4, 5, 6, 8, 9])
    # encode data
    data_label_encoder(data, [1, 6, 7, 8, 9])
    freq_encoder(data, [1])
